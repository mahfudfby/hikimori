...
════════════════════════════════════════ */
const J={
  ink:'#0b0d14', inkD:'#070810', inkM:'#12151f',
  indigo:'#1a1f38', indigoL:'#252c50',
  sakura:'#f2a7b8', sakuraL:'#ffd6e0', sakuraD:'#d4788e',
  bamboo:'#4a7c59', bambooL:'#6aad7a', bambooD:'#2d5c3a',
  vermillion:'#c0392b', vermillionL:'#e8523f',
  gold:'#c9a84c', goldL:'#e8c86a', goldD:'#a07828',
  moon:'#e8e8f5', moonD:'#b0b0d0',
  washi:'#f5ead0',
  mist:'rgba(180,190,220,0.06)',
  deep:'#060810',
  navy:'#0d1220',
  slate:'#1a2035',
};
      ...
      <motion.div style={{display:'flex',width:'max-content'}}
        ...
        {doubled.map((item,i)=>(
          <div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'0 24px',flexShrink:0,whiteSpace:'nowrap'}}>
            ...
            <span style={{fontFamily:'var(--font-body)',fontSize:'0.72rem',fontWeight:700,color:'rgba(255,255,255,0.6)',textTransform:'uppercase',letterSpacing:'1.5px'}}>{item.label}</span>
    ...
    <Reveal direction="up" delay={index*0.09}>
      <InkRipple>
        <motion.div
          ...
          style={{borderRadius:16,overflow:'hidden',border:`1px solid rgba(192,57,43,${open?0.5:0.15})`,background:`linear-gradient(135deg,rgba(18,18,32,0.92),rgba(26,20,40,0.88))`,backdropFilter:'blur(16px)',transition:'all 0.3s'}}>
...
const Home:React.FC=()=>{
  ...
  return (
    <div ref={containerRef} style={{background:J.ink,minHeight:'100vh',overflowX:'hidden'}}>
      ...
      ══════════════════════════════════ */}
      <section style={{position:'relative',padding:'clamp(4rem,10vw,8rem) clamp(1rem,5vw,2rem)',overflow:'hidden',background:`linear-gradient(180deg,${J.ink} 0%,${J.navy} 50%,${J.ink} 100%)`}}>
        <SakuraCanvas/>
        {/* Cherry trees */}
          ...
          <div id="about-row" style={{display:'flex',flexDirection:'column',gap:'3rem',alignItems:'center'}}>
            ...
            <Reveal direction="right" delay={0.15}>
              <div style={{maxWidth:560}}>
                ...
                <h2 style={{fontFamily:'var(--font-display)',fontSize:'clamp(2.5rem,8vw,5rem)',lineHeight:0.9,marginBottom:'0.3rem',color:J.moon}}>ABOUT ME !</h2>
                {/* Red line accent */}
                ...
                <p style={{color:'rgba(232,232,245,0.8)',lineHeight:1.9,marginBottom:'1rem',fontSize:'clamp(0.88rem,2vw,1rem)'}}>{about.bio1}</p>
                <p style={{color:'rgba(232,232,245,0.65)',lineHeight:1.9,fontSize:'clamp(0.88rem,2vw,1rem)'}}>{about.bio2}</p>
        ...
        <div style={{position:'absolute',bottom:0,left:0,right:0,height:80,background:`linear-gradient(to top,${J.ink},transparent)`,pointerEvents:'none'}}/>
      </section>

      {/* ══════════════════════════════════
          SERTIFIKASI — 巻物 (MAKIMONO SCROLL)
      ══════════════════════════════════ */}
      <section style={{position:'relative',padding:'clamp(4rem,10vw,8rem) clamp(1rem,5vw,2rem)',overflow:'hidden',background:`linear-gradient(180deg,${J.ink} 0%,${J.inkM} 60%,${J.ink} 100%)`}}>
        {/* Torii gate bg */}
        ...
        <div style={{position:'absolute',bottom:0,left:0,right:0,height:80,background:`linear-gradient(to top,${J.inkM},transparent)`,pointerEvents:'none'}}/>
      </section>

      {/* ══════════════════════════════════
          EXPERIENCE — 道 (MICHI / THE PATH)
      ══════════════════════════════════ */}
      <section style={{position:'relative',padding:'clamp(4rem,10vw,8rem) clamp(1rem,5vw,2rem)',overflow:'hidden',background:`linear-gradient(180deg,${J.inkM} 0%,${J.indigo} 50%,${J.inkM} 100%)`}}>
        {/* Stars */}
        {[...Array(20)].map((_,i)=>(
                    ...
                    <TiltCard style={{flex:1,borderRadius:16}} intensity={5}>
                      <InkRipple>
                        <motion.div whileHover={{borderColor:`rgba(192,57,43,0.5)`,background:'rgba(26,22,45,0.9)'}}
                          style={{background:`rgba(18,16,30,0.85)`,border:`1px solid rgba(192,57,43,0.15)`,borderRadius:16,padding:'1.4rem clamp(1rem,3vw,1.8rem)',backdropFilter:'blur(12px)',transition:'all 0.3s',position:'relative',overflow:'hidden'}}>
                          {/* Ink splash bg */}
                            ...
                            <div style={{display:'flex',flexWrap:'wrap',gap:'0.35rem',position:'relative',zIndex:1}}>
                              {exp.tags.split(',').map(t=>t.trim()).filter(Boolean).map((tag,ti)=>(
                                <motion.span key={tag} initial={{opacity:0,scale:0.8}} whileInView={{opacity:1,scale:1}} transition={{delay:ti*0.04+i*0.08}}
                                  style={{background:'rgba(242,167,184,0.07)',border:`1px solid rgba(242,167,184,0.18)`,color:'rgba(242,167,184,0.75)',borderRadius:5,padding:'3px 10px',fontSize:'0.72rem',fontWeight:500}}>
                                  {tag}
                                </motion.span>
                              ))}
                            </div>
      ...
      ══════════════════════════════════ */}
      <section style={{position:'relative',padding:'clamp(4rem,10vw,8rem) clamp(1rem,5vw,2rem)',overflow:'hidden',background:`linear-gradient(180deg,${J.ink} 0%,${J.inkD} 60%,${J.ink} 100%)`}}>
        {/* Mt Fuji */}
                ...
                <TiltCard style={{borderRadius:18,height:'100%'}} intensity={12}>
                  <InkRipple>
                    <motion.div
                      ...
                      style={{background:`linear-gradient(135deg,rgba(16,14,26,0.92),rgba(26,20,40,0.85))`,border:`1px solid rgba(192,57,43,0.15)`,borderRadius:18,padding:'1.8rem',position:'relative',overflow:'hidden',height:'100%',backdropFilter:'blur(14px)',transition:'all 0.3s'}}>
                      {/* Kanji watermark */}
                      ...
                      <p style={{color:'rgba(232,232,245,0.6)',fontSize:'0.85rem',lineHeight:1.6,position:'relative',zIndex:1}}>{sk.desc}</p>
                      {/* Bottom shimmer */}
      ...
      {/* ══════════════════════════════════
          CTA — 月 (TSUKI / MOON)
      ══════════════════════════════════ */}
      <section style={{position:'relative',padding:'clamp(4rem,10vw,8rem) clamp(1rem,5vw,2rem)',textAlign:'center',overflow:'hidden',background:`linear-gradient(180deg,${J.ink} 0%,${J.inkD} 50%,${J.ink} 100%)`}}>
        {/* Full moon */}
        ...
        <div style={{position:'relative',zIndex:2,maxWidth:700,margin:'0 auto'}}>
          <Reveal direction="scale">
            ...
            <h2 style={{fontFamily:'var(--font-display)',fontSize:'clamp(2rem,8vw,5rem)',lineHeight:0.95,color:J.moon,wordBreak:'break-word',marginBottom:'1rem',textShadow:`0 0 40px rgba(232,232,245,0.1)`}}>
              SIAP BERKOLABORASI?
            </h2>
            {/* Red divider */}
